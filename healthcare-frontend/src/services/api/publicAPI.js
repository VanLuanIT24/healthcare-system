...
const publicAPI = {
  ...
  getStats: async () => {
    return await axios.get('/api/public/stats');
  },

  // 👨‍⚕️ Lấy danh sách bác sĩ nổi bật cho carousel
  getFeaturedDoctors: async (limit = 4) => {
    return await axios.get('/api/public/featured-doctors', {
      params: { limit }
    });
  },
  ...
  getAllDoctors: async (filters = {}) => {
    return await axios.get('/api/public/doctors', {
      params: {
  ...
  getDoctors: async (params = {}) => {
    return await axios.get('/api/public/doctors', { params });
  },

  // 🏥 Lấy chi tiết một bác sĩ
  getDoctorDetail: async (doctorId) => {
    return await axios.get(`/api/public/doctors/${doctorId}`);
  },

  // 📋 Lấy danh sách chuyên khoa
  getSpecialties: async () => {
    return await axios.get('/api/public/specialties');
  },

  // 🏢 Lấy danh sách khoa/phòng ban
  getDepartments: async () => {
    return await axios.get('/api/public/departments');
  }
};
