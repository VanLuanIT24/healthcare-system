...
const auditLogSchema = new mongoose.Schema({
  ...
  ipAddress: {
    type: String,
    required: false, // ✅ SỬA: required: true -> false
    default: '0.0.0.0', // ✅ THÊM default value
  },
  userAgent: String,
  httpMethod: {
    type: String,
...
auditLogSchema.statics = {
  /**
   * TẠO AUDIT LOG MỚI (SAFE VERSION)
   */
  async logAction(logData) {
    try {
      ...
      const auditLog = new this(safeLogData);
...
auditLogSchema.pre('save', function(next) {
  ...
  // ✅ ĐẢM BẢO CÓ GIÁ TRỊ MẶC ĐỊNH CHO CÁC TRƯỜNG REQUIRED
  if (!this.ipAddress) this.ipAddress = '0.0.0.0';
  if (!this.resource) this.resource = 'User';
