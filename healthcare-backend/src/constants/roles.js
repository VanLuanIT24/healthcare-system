...
/**
 * 🏥 HỆ THỐNG PHÂN QUYỀN RBAC CHO ỨNG DỤNG Y TẾ
 * Tác giả: Healthcare Development Team
 * Phiên bản: 2.0 - Đã refine bảo mật & thực tế hơn (22/12/2025)
 * 
 * Các thay đổi chính so với bản gốc:
 * - HOSPITAL_ADMIN không được tạo HOSPITAL_ADMIN khác (chỉ SUPER_ADMIN mới được)
 * - Thêm PATIENT.UPDATE_ADMINISTRATIVE cho lễ tân cập nhật thông tin hành chính
 * - DEPARTMENT_HEAD được register DOCTOR và NURSE trong khoa
 * - Tách rõ APPOINTMENT.REQUEST_CANCEL (chỉ bệnh nhân) và CONFIRM_CANCEL
 * - CLINICAL_ADMIN có thêm quyền register bác sĩ cao cấp
 * - Cải thiện comment và cấu trúc
 */

const ROLES = Object.freeze({
  // ===== QUẢN TRỊ HỆ THỐNG =====
  SUPER_ADMIN: 'SUPER_ADMIN',         // Quyền cao nhất (dev ops)
  SYSTEM_ADMIN: 'SYSTEM_ADMIN',       // Quản trị hệ thống (không liên quan y tế)
  CLINICAL_ADMIN: 'CLINICAL_ADMIN',   // Bác sĩ trưởng cao cấp

  // ===== QUẢN TRỊ BỆNH VIỆN & KHOA =====
  HOSPITAL_ADMIN: 'HOSPITAL_ADMIN',   // Quản trị viên bệnh viện
...
const PERMISSIONS = Object.freeze({
  ...
  // Đăng ký người dùng (chỉ cấp cho vai trò cao)
  ...
  'AUTH_REGISTER_HOSPITAL_ADMIN': 'AUTH.REGISTER_HOSPITAL_ADMIN', // Chỉ SUPER_ADMIN
  'AUTH_REGISTER_CONSULTANT_SUPPORT': 'AUTH.REGISTER_CONSULTANT_SUPPORT',
  ...
  'PATIENT_UPDATE': 'PATIENT.UPDATE', // Update y tế (bác sĩ)
  'PATIENT_UPDATE_ADMINISTRATIVE': 'PATIENT.UPDATE_ADMINISTRATIVE', // Update hành chính (lễ tân)
  ...
  'APPOINTMENT_REQUEST_CANCEL': 'APPOINTMENT.REQUEST_CANCEL',   // Bệnh nhân yêu cầu hủy
  'APPOINTMENT_CONFIRM_CANCEL': 'APPOINTMENT.CONFIRM_CANCEL',   // Nhân viên duyệt hủy
...
}); // ← ĐÃ SỬA: Đóng object PERMISSIONS đúng cách

// ===== PHÂN QUYỀN THEO TỪNG VAI TRÒ =====
const ROLE_PERMISSIONS = Object.freeze({
  [ROLES.SUPER_ADMIN]: [...Object.values(PERMISSIONS)], // Toàn quyền
  ...
  [ROLES.HOSPITAL_ADMIN]: [
    ...
    PERMISSIONS.AUTH_REGISTER_DOCTOR, PERMISSIONS.AUTH_REGISTER_DEPARTMENT_HEAD,
    // Không có AUTH.REGISTER_HOSPITAL_ADMIN → bảo mật
  ...
  [ROLES.CONSULTANT_SUPPORT]: [
    PERMISSIONS.AUTH_LOGIN, PERMISSIONS.AUTH_LOGOUT,
    PERMISSIONS.USER_VIEW,
    PERMISSIONS.PATIENT_VIEW, PERMISSIONS.PATIENT_UPDATE_ADMINISTRATIVE,
    PERMISSIONS.CONSULTATION_VIEW, PERMISSIONS.CONSULTATION_CREATE, PERMISSIONS.CONSULTATION_UPDATE,
...
const ROLE_WEIGHTS = Object.freeze({
  ...
  [ROLES.CONSULTANT_SUPPORT]: 10,
  [ROLES.RECEPTIONIST]: 11,
  [ROLES.PATIENT]: 12,
  [ROLES.GUEST]: 13,
});
...
function hasPermission(role, permission) {
  if (!role || !permission) {
    console.warn('❌ [RBAC] Thiếu role hoặc permission:', { role, permission });
    return false;
  }
  const permissions = ROLE_PERMISSIONS[role];
  if (!permissions) {
    console.warn('❌ [RBAC] Không tìm thấy vai trò:', role);
    return false;
  }
...
// ===== DEBUG KHI CHẠY TRỰC TIẾP FILE =====
if (require.main === module) {
  console.log('=== KIỂM TRA HỆ THỐNG RBAC ===\n');
  const testCases = [
  ...
  testCases.forEach((test, i) => {
    ...
    console.log(`${status} Test ${i + 1}: ${test.current} có thể tạo ${test.target} → ${result}`);
  });
}