...
class DashboardService {
   ...
   */
  static async getOverview(patientId) {
    const [
   ...
   */
  static async getHealthStats(patientId, days = 30) {
    ...
    visits.forEach((visit) => {
      if (visit.vitalSigns) {
        ...
        if (visit.vitalSigns.bmi) vitalStats.bmi.push(visit.vitalSigns.bmi);
      }
    });
   ...
   * Get health alerts
   */
  static async getHealthAlerts(patientId) {
    const alerts = [];
   ...
   */
  static async getActivityHistory(patientId, limit = 20) {
    const activities = [];
